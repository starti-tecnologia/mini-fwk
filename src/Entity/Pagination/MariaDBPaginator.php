...
class MariaDBPaginator
{
    ...
    private function runPaginatorSelect(array $options)
    {
        ...
        $sqls = $this->createPaginatorSelects($options);
        $resultStm = $options['connectionInstance']->prepare($sqls[0]);
        $resultStm->execute($bindings);
    ...
    public function createPaginatorSelects(array $options)
    {
        ...
        $resultSql = "$initialSql LIMIT $skip,$perPage";

        $countSql = preg_replace(
            $from
                ? '/^SELECT (.*?)FROM (' . preg_quote($from) . ')(.*)$/i'
                : '/^SELECT (.*?)FROM(.*)$/i',
            ($from ? 'SELECT COUNT(1) FROM $2$3' : 'SELECT COUNT(1) FROM $2'),
            $initialSql
        );

        return [$resultSql, $countSql];
    }
}
